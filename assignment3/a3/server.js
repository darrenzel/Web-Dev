...
io.on('connection', function(socket){
  ...
  socket.on('redBoxData', function(data){
    console.log('RECEIVED BOX DATA: ' + data)
    //to broadcast message to everyone including sender:
    io.emit('redBoxData', data) //broadcast to everyone including sender
  })
    socket.on('puck', function(data){
    console.log('RECEIVED BOX DATA: ' + data)
    //to broadcast message to everyone including sender:
    io.emit('puck', data) //broadcast to everyone including sender
  })
  socket.on('red', function(data){
    console.log('RECEIVED BOX DATA: ' + data)
    //to broadcast message to everyone including sender:
    io.emit('red', data) //broadcast to everyone including sender
  })
    socket.on('blue', function(data){
    console.log('RECEIVED BOX DATA: ' + data)
    //to broadcast message to everyone including sender:
    io.emit('blue', data) //broadcast to everyone including sender
  })
})
...
console.log("Server Running at PORT 3000 CNTL-C to quit, access http://localhost:3000/assignment3.html to test");
