...
function read(title){
  var file = fs.readFileSync('songs/'+title+'.txt').toString();
  return file;
}

function savefile(file, newfilename){
  fs.writeFile("songs/"+newfilename+".txt", file, function(error){
    if(error){
...
var peacefulEasyFeeling = read('Peaceful Easy Feeling');
var sisterGoldenHair = read('Sister Golden Hair');
var brownEyedGirl = read('Brown Eyed Girl');
  ...
  .createServer(function(request, response) {
    ...
    request.on("end", function() {
      ...
      if (request.method == "POST") {
        ...
        if(dataObj.text=="save"){
          savefile(dataObj.file, dataObj.newfilename);
          //returnObj.text = "file saved at "+dataObj.newfilename+".txt";
